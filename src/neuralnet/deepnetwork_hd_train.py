...
for operation in operations_to_run:
    ...
    save_dir = f"/home/rahul3/scratch/2026/experiments/shallownetwork/{operation}/shallow_matrixnet_{ID}"
    ...
    for dim in range(1, 8):
        ...
        for train_sample in train_samples:
            ...
            model_save_dict_path = os.path.join(model_save_dir, f'{operation}_model_{str(train_sample)}.pth')
            torch.save(model.state_dict(), model_save_dict_path)
            logger.info(f"Model saved to {model_save_dict_path}")
            ...
            with torch.no_grad():
                for test_idx, (x, y) in enumerate(test_loader):
                    ...
                    logger.info(f"Test: Batch {test_idx+1}/{len(test_loader)}, Current Test Batch Loss: {criterion(output, y).item():.4f}, Cumulative Test Loss: {test_loss:.4f}")
            ...
            # Collect results in a list instead of appending to DataFrame
            results_list = []
            ...
            with torch.no_grad():
                ...
                for tol in tols:
                    ...
                    results_list.append({
                        ...
                        "tolerance": tol,
                        "correct_predictions": evaluations_correct[tol]/evaluation_samples,
                        "accuracy": accuracy
                    })
            
            # Create DataFrame from the list
            df = pd.DataFrame(results_list)
            df.to_csv(os.path.join(model_save_dir, f'{operation}_results_{str(train_sample)}.csv'), index=False)
            logger.info(f"Results saved to {os.path.join(model_save_dir, f'{operation}_results_{str(train_sample)}.csv')}")
